...
Read off the git history rather than asserted. Reproduce with `git shortlog -sne main`.

| Person | Contribution |
| :--- | :--- |
| **Jasper Ang** | Repository and kit setup, phase 0 diagnostics, the day one scaffold and the frozen `src/interfaces.py` seams, the agent shell and platform, the evaluation harness, the offline LLM semantic prior and the live reranker, and integration of every branch into `main`. |
| **Jarell Liaw** | Field-aware retrieval: per-field TF-IDF, the `phrases` argument on the retriever seam, and `evaluation/rank_diagnostics.py`, reaching HitRate 0.988 in isolation. The Devpost description and the first demo script. |
| **Gan Ziheng** | The profile preference-tags ranking experiment: built, swept, measured as no gain outside the noise band, and removed rather than shipped. Also found the cross-machine score drift that led to the deterministic tie-break fix. |
| **Chee Hin** | Exact dependency pinning, correcting the shipped-retrieval description in the limitations, and rebuilding the demo script around measured rather than assumed runtimes. |

Two of these are negative results, listed as contributions deliberately. The preference-tags
experiment and the field-aware sweep both cost real work and both ended in "measured, does not
help, not shipped". Recording them is what stops the next person rebuilding them.
...
| File | What it holds |
| :--- | :--- |
| `docs/consolidation.md` | Single reference: every design decision, its evidence, and what is left |
| `docs/retrieval-merge-finding.md` | Every retrieval and ranking experiment with its numbers, including the rejected ones |
| `docs/build-plan.md` | The four-role split and the day-by-day plan |
| `docs/handoff.md` | State of the project for someone picking it up |
| `docs/llm-semantic-stage.md` | How the offline prior is built and what it is worth |
| `docs/demo-script-v2.md` | Demo video script, organised by pillar |
| `phase0-findings.md` | The recall ceiling and attribute coverage measured before any policy was written |
